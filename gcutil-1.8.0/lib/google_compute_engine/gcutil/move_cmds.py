...
class MoveInstancesBase(command_base.GoogleComputeCommand):
  ...
  def SetApi(self, api):
    self._addresses_api = None
    if hasattr(api, 'addresses'):
  ...
  def Handle(self, *args, **kwargs):
    ...
    if self._addresses_api:
      self._address_list = utils.AllAggregated(
          self._addresses_api.aggregatedList,
          self._project,
          'addresses')
  ...
  def _CreateInstances(self, instances, src_zone, dest_zone):
    ...
    if self._IsUsingAtLeastApiVersion('v1beta15'):
      ip_addresses = set(self._address_list.get('items', []))
  ...
  def _WaitForSnapshots(self, snapshots):
    ...
    while True:
      if timeout_seconds >= 0 and time.time() - start_sec > timeout_seconds:
        raise command_base.CommandError(
      ...
      time.sleep(sleep_between_polls_seconds)
  ...
  def _CreateSnapshots(self, snapshot_mappings, src_zone, dest_zone):
    ...
    for disk_name, snapshot_name in snapshot_mappings.iteritems():
      ...
      if self._IsUsingAtLeastApiVersion('v1beta15'):
        requests.append(self._disks_api.createSnapshot(
            project=self._project,
            zone=src_zone,
            disk=disk_name,
  ...
  def _CheckDeprecatedResources(self, instances_to_move):
    ...
    if errors:
      ...
      raise command_base.CommandError(message.getvalue())
...
class MoveInstances(MoveInstancesBase):
  ...
  def _CheckDestinationZone(self):
    ...
    zone = self._zones_api.get(project=self._project, zone=zone_name).execute()
    ...
    if maintenance_window is not None:
      ...
      if delta < datetime.timedelta(weeks=2):
        warning = 'Zone %s will enterin maintenance in'
        if delta.days:
          warning += ' %d day%s' % (
