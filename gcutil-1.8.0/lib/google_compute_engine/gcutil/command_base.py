...
class Presenter(object):
  ...
  def PromptForImage(self, images_api):
    ...
    """
    projects = list(set([self._flags.project] + STANDARD_IMAGE_PROJECTS))
  ...
  def PromptForKernel(self, kernels_api):
    ...
    return self.PromptForChoice(choices, 'kernel')
  ...
  def PromptForMachineType(self, machine_types_api, for_test_auto_select=False):
    ...
    def MachineTypePrompt(resource):
      name = self.PresentElement(resource['selfLink'])
  ...
  def PresentElement(self, field_value):
    ...
    if isinstance(field_value, basestring):
      ...
      field_value = self.StripBaseUrl(field_value).strip('/')
      if field_value.startswith('projects/' + self._flags.project):
        ...
        if len(field_value_parts) == 2:
          # projects/<project>
          field_value = field_value_parts[-1]

        elif len(field_value_parts) >= 5 and field_value_parts[2] == 'global':
          # projects/<project>/global/<collection>/<resource>
          field_value = '/'.join(field_value_parts[4:])

        elif (len(field_value_parts) >= 4 and
              field_value_parts[2] in ('zones', 'regions')):
          # projects/<project>/(zones|regions)/<zone|region>
          # [/<collection>/<resource>]

          if len(field_value_parts) == 4:
            # projects/<project>/(zones|regions)/<zone|region>
            # Return region/zone name only.
            field_value = field_value_parts[-1]

          elif len(field_value_parts) >= 6:
            # projects/<project>/(zones|regions)/<zone|region>
            # /<collection>/<resource>

            if field_value_parts[2] == 'zones':
              scope_type = 'zone'
            else:
              scope_type = 'region'
            ...
            if hasattr(self._flags, scope_type):
              ...
              # projects/<project>/scope/<scope>/<collection/...
              if scope_name == field_value_parts[3]:
                field_value = '/'.join(field_value_parts[5:])
              else:
...
class ApiThreadPoolOperation(thread_pool.Operation):
  ...
  def __init__(self, request, command, wait_for_operation,
               timeout_seconds, sleep_between_polls_seconds,
  ...
  def Run(self):
    ...
    if self._wait_for_operation:
      result = self._command.WaitForOperation(
          ...
          time, result, http=http, collection_name=self._collection_name)
    return result
...
class GoogleComputeCommand(appcommands.Cmd):
  ...
  def _GetZone(self, zone=None):
    ...
    """
    if zone is None:
      zone_resource = self._presenter.PromptForZone(self._zones_api)
  ...
  def _GetRegions(self):
    ...
    """
    return utils.AllNames(self._regions_api.list, self._project)
  ...
  def NormalizeMachineTypeResourceName(self, project, zone, collection,
    ...
    Returns:
      The full URL of the resource.
    """
    if self._IsUsingAtLeastApiVersion('v1beta15') and zone:
  ...
  def GetRegionForResource(self, api, resource_name, fail_if_not_found=True):
    ...
    items = []
    for region in self._GetRegions():
    ...
    if len(items) == 1:
      ...
      LOGGER.info('Region for %s detected as %s.', repr(resource_name),
                  repr(region or GLOBAL_SCOPE_NAME))
      LOGGER.warning('Consider passing \'--region=%s\' to avoid the unnecessary '
                     'region lookup which requires extra API calls.',
                     region or GLOBAL_SCOPE_NAME)
      return region

    if fail_if_not_found:
      raise CommandError('Could not determine the region of \'%s\'.' %
                         resource_name)
    else:
      return None
  ...
  def RunWithFlagsAndPositionalArgs(self, flag_values, pos_arg_values):
    ...
    if self._IsUsingAtLeastApiVersion('v1beta15'):
      self._region_operations_api = compute_api.regionOperations()
  ...
  def WaitForOperation(self, timeout_seconds, sleep_between_polls_seconds,
    ...
    while result['status'] != 'DONE':
      ...
      LOGGER.info('Waiting for %s of %s. Sleeping for %ss.', operation_type,
                  qualified_name, sleep_between_polls_seconds)
      ...
      if self._IsUsingAtLeastApiVersion('v1beta15'):
        ...
        if scope_type == 'zones':
          ...
          poll_api = self._region_operations_api
  ...
  def _PrintAggregatedList(self, result):
    ...
    if 'items' in result:
      for scope, obj in result['items'].iteritems():
        collection_name = self.resource_collection_name
        header = '\n\nItems in %s:\n' % scope
        if collection_name in obj:
          res = [r for r in obj[collection_name]]
          self._CreateAndPrintTable(res, header, self.GetPrintSpec().summary)
        elif 'warning' in obj:
          warning = [obj['warning']]
          self._CreateAndPrintTable(warning, header,
                                    self._warning_print_spec.detail)
...
class GoogleComputeListCommand(GoogleComputeCommand):
  ...
  def Handle(self):
    ...
    # Aggregated list.
    if (self._IsUsingAtLeastApiVersion('v1beta15') and
