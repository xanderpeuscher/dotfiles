    ...
    while True:
      ...
      if max_results is not None:
        if len(results['items']) + len(items) >= max_results:
          break
        if max_results - len(items) - len(results['items']) < 100:
          params['maxResults'] = (
...
def AllAggregated(func, project_or_list, collection, max_results=None,
                  filter=None):
    ...
    while True:
      ...
      if max_results is not None:
        if count >= max_results:
          break
        if max_results - count < 100:
          params['maxResults'] = max_results - count
  ...
  for _, contents in aggregated_list['items'].iteritems():
    if collection in contents and 'warning' in contents:
      del contents['warning']

  if max_results is not None:
    aggregated_list['items'] = aggregated_list['items'][:max_results]
